...
OAuth.setProperties(OAuth, {
    ...
    setParameter: function setParameter(message, name, value) {
        ...
        if (parameters instanceof Array) {
            for (var p = 0; p < parameters.length; ++p) if (parameters[p][0] == name) if (value === undefined) parameters.splice(p, 1); else {
    ...
    completeRequest: function completeRequest(message, accessor) {
        ...
        map.oauth_consumer_key == null && OAuth.setParameter(message, "oauth_consumer_key", accessor.consumerKey || "");
        map.oauth_token == null && accessor.token != null && OAuth.setParameter(message, "oauth_token", accessor.token);
        map.oauth_version == null && OAuth.setParameter(message, "oauth_version", "1.0");
        map.oauth_timestamp == null && OAuth.setParameter(message, "oauth_timestamp", OAuth.timestamp());
        map.oauth_nonce == null && OAuth.setParameter(message, "oauth_nonce", OAuth.nonce(6));
...
OAuth.setProperties(OAuth.SignatureMethod.prototype, {
    ...
    initialize: function initialize(name, accessor) {
        var consumerSecret;
        accessor.accessorSecret != null && name.length > 9 && name.substring(name.length - 9) == "-Accessor" ? consumerSecret = accessor.accessorSecret : consumerSecret = accessor.consumerSecret;
...
var OAuthAdapter = function(pConsumerSecret, pConsumerKey, pSignatureMethod) {
    function showLoading() {
        ...
        estimates[estimateID] || (estimates[estimateID] = firstLoad ? 2000 : 1000);
        firstLoad = !1;
            ...
            if (!adapter.isAuthorized()) {
                function receivePin() {
                    adapter.getAccessToken(urls.accessToken, function(evt) {
                        if (!evt.success) alert("Twitter did not give us an access token!"); else {
